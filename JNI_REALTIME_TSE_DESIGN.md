...
> - **Optimization Applied (2026-06-13):** To resolve the high computation overhead of running inference every 10ms (which requires re-running context window calculations), the native engine compiles and processes audio in 320ms chunks. It accumulates 320ms (32 hops of 10ms) before running TSE inference once, significantly improving RTF (Real-Time Factor) and reducing CPU/JNI overhead.
