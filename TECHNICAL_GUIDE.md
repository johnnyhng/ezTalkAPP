...
*   **主畫面模式 (`HOME`)：**
    ...
    *   **Autoplay Queue：** 若設定啟用 `autoplay`，且 `enableHomeLlmCorrection = true`、`enableTtsFeedback = false`，只有 `confidence >= 0.9` 的 correction 結果會進入 autoplay queue，等待安全空檔後自動朗讀並確認。
...
7.  若 `autoplay` 啟用，且 correction `confidence >= 0.9`，該句不會立刻播放，而是先進入 autoplay queue。
8.  autoplay queue 只會在 `!isRecognizingSpeech`、`countdownProgress == 0`、沒有 WAV 播放、沒有任何 TTS 播放、且 ASR model 不在 loading 時開始播放。
9.  若 autoplay 開始前 Home 原本正在錄音，系統會先暫停錄音；播放完成後再恢復原本的錄音狀態。
...
Home 的 autoplay 是建立在 LLM correction 之上的第二層背景流程：

*   設定項目為 **Autoplay**。
*   只有在 `autoplay = true`、`enableHomeLlmCorrection = true`、`enableTtsFeedback = false` 時才會生效。
*   correction 雖然在 `confidence >= 0.85` 時就可能自動套用，但只有 `confidence >= 0.9` 的結果才會進入 autoplay queue。
*   queue 以 `wavFilePath` 去重；同一筆若後續有更新的高信心 correction，會覆蓋 queue 中既有的待播內容。
*   autoplay 完成後會把該筆 transcript 標記為 `checked = true`，但不會像 feedback flow 那樣把 `mutable` 改成 `false`。
...
| **autoplay** | `UserSettings` | `false` | 啟用 Home autoplay queue。只會朗讀 `confidence >= 0.9` 的 Home correction 結果，並在播放前後暫停 / 恢復原本的錄音狀態。 |
